...
from z3 import (And, Xor, Not, Or, is_bool, is_bv, is_bv_value, is_false, is_true,
                simplify, unsat)
...
from eunomia.arch.wasm.lib.utils import is_modeled, _extract_params
...
class ControlInstructions:
    ...
    def deal_with_call(self, state, f_offset, data_section, analyzer, lvar):
        ...
        readable_callee_func_name = readable_internal_func_name(
            ...
            callee_func_name)
        if Configuration.get_dsl_flag() and readable_callee_func_name.find('$') != -1:
            name = readable_callee_func_name
            if name in Configuration._aes_asserts:
                params = _extract_params(param_str, state)
                locvar = {'____'+str(i): v for i,v in enumerate(params)}
                exec(Configuration._aes_asserts[name], globals(), locvar)
                state.solver.add(locvar['cons'])
            states = [state]
