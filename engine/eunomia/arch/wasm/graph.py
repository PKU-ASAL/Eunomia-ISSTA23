...
class Graph:
    ...
    def initialize(cls):
        ...
        def init_aes_func():
            ...
            """
            script_path = Configuration.get_file_path()[:-4]+'ll.stmt'
            aes_guides = {}
            aes_asserts = {}
            aes_vars = set()
            with open(script_path, 'r') as rfh:
                s = rfh.read().split('!!!')[:-1]
                for item in s:
                    name, script = item.split('|||')
                    name = name.strip()
                    script = script.strip()
                    ...
                    while True:
                        try:
                            exec(script, globals(), delta)
                            break;
                        except NameError as e:
                            delta[str(e).split("'")[1]] = 0
                    if 'cons' in delta:
                        aes_asserts[name] = script
                    else:
                        aes_guides[name] = script 
                    aes_vars |= set(delta.keys())
            Configuration._aes_guides = aes_guides
            Configuration._aes_asserts = aes_asserts
            Configuration._aes_vars = {k:0 for k in aes_vars}
            for bb_name, instructions in cls.bb_to_instructions.items():
                for instr in instructions:
                    if instr.name == 'call':  # aes rules will be regarded as instrumented function calls
                        ...
                        if len(readable_name.split('$')) == 2 and readable_name in aes_guides:
    ...
    def visit_interval(cls, states, blk, heads, prev=None):
        ...
        def unique_idx():
            ...
            counter += 1
            return counter-1
        default_cons_prior = copy.deepcopy(Configuration._aes_vars)
    ...
    def aes_run_local(cls, lvar, blk):
        ...
        for name in cls.aes_func[blk]:
            exec(Configuration._aes_guides[name], new_lvar)
            del new_lvar['__builtins__']
            if new_lvar['halt'] == 1:
                new_lvar['prior'] = -65536
        return new_lvar
